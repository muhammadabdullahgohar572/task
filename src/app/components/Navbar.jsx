...
import { contextcode } from "../context/contextfile";
import { useRouter } from "next/navigation";

export const Navbar = () => {
  const { UserData, setuser } = useContext(contextcode);
  const router = useRouter();
  const logout = async () => {
    try {
      const apicall = await fetch("/api/Logout", { method: "POST" });
      setuser(null);
      router.push("/");
    } catch (error) {
      console.log(error);
        ...
        {UserData ? (
          ...
          <>
            <Link href={"/pages/Login"}>
              <button className="flex items-center gap-2 px-5 py-2 rounded-lg border border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white transition duration-200">
                <LogIn size={18} /> Login
              </button>
            </Link>
